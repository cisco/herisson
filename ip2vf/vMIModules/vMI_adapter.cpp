        ...
        case CMD_TICK:
            {
                ...
                */
                ...
                nb_output = libvMI_get_output_count(g_vMIModule);
                LOG("receive frame #%d on input[%d], fmt=%s, size=%d bytes, nb_output=%d, frm=0x%x", hFrame, in, (fmt == 1 ? "video" : "audio"), size, nb_output, frm);
